      ...
      <Paper elevation={4} sx={{ overflowX: "auto" }}>
        <Table>
          ...
          <TableBody>
            {productos.map((item) => {
              const impuesto =
                item.precio_unitario && item.porcentaje
                  ? ((item.precio_unitario * parseFloat(item.porcentaje)) /
                      100) *
                    item.cantidad
                  : null;
              ...
              return (
                ...
                >
                  ...
                  <TableCell>
                    {item.costo_base != null
                      ? `₡${parseFloat(item.costo_base).toLocaleString()}`
                      : "-"}
                  </TableCell>
                  <TableCell>
        ...
        <DialogContent dividers>
          {selectedProduct && (
            <Box sx={{ lineHeight: 1.8 }}>
              ...
              <Typography>
                <Label>Base ₡:</Label> ₡
                {selectedProduct.costo_base?.toLocaleString() ?? "-"}
              </Typography>
              <Typography>
                <Label>Adicional ₡:</Label> ₡
                {selectedProduct.costo_adicional?.toLocaleString() ?? "-"}
              </Typography>
              <Typography>
                <Label>Precio Unitario ₡:</Label> ₡
                {selectedProduct.precio_unitario?.toLocaleString() ?? "-"}
              </Typography>
              <Typography>
              ...
              <Typography>
                ...
                {selectedProduct.subtotal?.toLocaleString() ?? "-"}
              </Typography>
              {selectedProduct.precio_unitario &&
                selectedProduct.porcentaje && (
                  <Typography>
                    <Label>Impuesto ({selectedProduct.porcentaje}%):</Label> ₡
                    {(
                      selectedProduct.precio_unitario *
                      (parseFloat(selectedProduct.porcentaje) / 100) *
