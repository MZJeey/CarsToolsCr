...
const parseOpciones = (raw) => {
  ...
  if (typeof data === "object") {
    ...
    return Object.entries(data).map(([criterio, v]) => norm({ criterio, ...v }));
  }
...
const TodosProductosPersonalizados = () => {
  ...
  // TODO: Reemplaza con el ID del pedido actual (de tu contexto/ruta)
  const pedidoIdActual = 1;
  ...
  item.opciones_personalizacion ?? item.opciones_personalizacion_json
);
        ...
        <DialogContent dividers>
          {selectedProduct && (
            <Box sx={{ lineHeight: 1.8 }}>
              <Typography><Label>Nombre:</Label> {selectedProduct.nombre_personalizado}</Typography>
              <Typography><Label>Producto Base:</Label> {selectedProduct.nombre_producto_base}</Typography>
              <Typography><Label>Base ₡:</Label> {formatCRC(selectedProduct.costo_base)}</Typography>
              <Typography><Label>Adicional ₡:</Label> {formatCRC(selectedProduct.costo_adicional)}</Typography>
              <Typography><Label>Precio Unitario ₡:</Label> {formatCRC(selectedProduct.precio_unitario)}</Typography>
              <Typography><Label>Cantidad:</Label> {selectedProduct.cantidad}</Typography>
              <Typography><Label>Subtotal ₡:</Label> {formatCRC(selectedProduct.subtotal)}</Typography>
              <Divider sx={{ my: 2 }} />
              <Typography variant="subtitle1" fontWeight="bold">Personalizaciones:</Typography>
              <Box sx={{ mt: 1, display: "flex", flexWrap: "wrap" }}>
                {parseOpciones(selectedProduct.opciones_personalizacion).map((o, idx) => (
                  <Chip
                    key={idx}
                    label={`${o.criterio}: ${o.opcion} (+${formatCRC(o.costo)})`}
                    sx={{ m: 0.3 }}
